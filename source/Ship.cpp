...
void Ship::ChangeGovernment(const Government *next, bool changeSwizzle, bool clearLooted)
{
	government = next;
	if(changeSwizzle)
		SetSwizzle(customSwizzle >= 0 ? customSwizzle : government->GetSwizzle());
...
shared_ptr<Ship> Ship::Board(bool autoPlunder, bool nonDocking)
{
	...
	if(CanBeCarried() && !nonDocking)
	{
		...
		{
			if(victim->lootedGovernment == government)
				victim->MakeLooted();
...
int Ship::WasCaptured(const shared_ptr<Ship> &capturer)
{
	...
	// Set the new government, but do not repaint ship.
	ChangeGovernment(capturer->government, false, true);
